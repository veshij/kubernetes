...
func (dc *DisruptionController) podProcessorWorker(ctx context.Context) {
	for dc.processPodItem() {
	}
}

func (dc *DisruptionController) processPodItem() bool {
	key, shutdown := dc.pdbUpdatePodQueue.Get()
	if shutdown {
		return false
	}

	defer dc.pdbUpdatePodQueue.Done(key)

	dc.lock.Lock()
	pod, ok := dc.podCache[key.(string)]
	delete(dc.podCache, key.(string))
	dc.lock.Unlock()
	if !ok {
		klog.Errorf("Pod %q not found in cache", key)
		return true
	}
...
func (dc *DisruptionController) processPod(pod *v1.Pod) {
